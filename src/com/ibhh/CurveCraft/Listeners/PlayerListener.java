...
public class PlayerListener implements Listener
{
	...
	public void precommand(PlayerCommandPreprocessEvent event)
	{
		...
		if((event.getMessage().toLowerCase().startsWith("/cc".toLowerCase())) && (this.plugin.getConfigHandler().getConfig().getBoolean("debugfile")) && (this.plugin.getPrivacy().getConfig().containsKey(event.getPlayer().getName())))
		{
	...
	{
		long time = System.nanoTime();
		if(this.plugin.getArenaHandler().getArenaOfPlayer(e.getPlayer()) != null && !e.getPlayer().getWorld().equals(plugin.getArenaHandler().getArenaOfPlayer(e.getPlayer()).getCorner1().getWorld()))
		{
			try
			{
	...
	{
		long time = System.nanoTime();
		if(((e.getEntity() instanceof Player)) && (this.plugin.getArenaHandler().getArenaOfPlayer((Player) e.getEntity()) != null))
		{
			e.setCancelled(true);
		}
	...
	{
		long time = System.nanoTime();
		if(((e.getExited() instanceof Player)) && (this.plugin.getArenaHandler().getArenaOfPlayer((Player) e.getExited()) != null))
		{
			e.setCancelled(true);
		}
