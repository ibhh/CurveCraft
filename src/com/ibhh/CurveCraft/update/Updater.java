...
public class Updater
{
	...
	private static final String HOST = "https://api.curseforge.com"; // Slugs will be appended to this to get to the project's
// RSS feed

	private static final String USER_AGENT = "Updater (by Gravity)";
	private static final String delimiter = "^v|[\\s_-]v"; // Used for locating version numbers in file names
	private static final String[] NO_UPDATE_TAG = {"-DEV", "-PRE", "-SNAPSHOT"}; // If the version number contains one of these,
// don't update.
	private static final int BYTE_SIZE = 1024; // Used for downloading files
	private final YamlConfiguration config = new YamlConfiguration(); // Config file
	private String updateFolder;// The folder that downloads will be placed in
	private Updater.UpdateResult result = Updater.UpdateResult.SUCCESS; // Used for determining the outcome of the update
// process
		...
		try
		{
			...
			for(final File dFile : new File(zipPath).listFiles())
			{
				if(dFile.isDirectory())
				{
					...
					{
						final File oFile = new File(this.plugin.getDataFolder().getParent(), dFile.getName()); // Get current
// dir
	 ...
	 * </p>
	 * <p>
	 * Updater will call this method from {@link #versionCheck(String)} before deciding whether the remote version is actually
	 * an update. If you have a specific versioning scheme with which a mathematical determination can be reliably made to
	 * decide whether one version is higher than another, you may revise this method, using the local and remote version
