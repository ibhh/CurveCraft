...
public class ArenaHandler
{
	...
	public void loadArena(File file, World world)
	{
		if(file.exists())
		{
			CCArena cf = new CCArena(plugin, file, world);
	...
	public void delete(CCArena a)
	{
		File file = new File(plugin.getDataFolder() + File.separator + "arena-saves" + File.separator + a.getCorner1().getWorld().getName() + File.separator + a.getName() + ".yml");
		if(file.exists())
		{
			arena.remove(a);
			plugin.getLoggerUtility().log("Arena \"" + file.getName().replace(".yml", "") + "\" in world \"" + a.getCorner1().getWorld().getName() + "\" permanently deleted!", LoggerUtility.Level.INFO);
		}
	}
	...
	{
		return (ArrayList<CCArena>) arena.clone();
	}
