package com.ibhh.CurveCraft.logger;
...
public class LoggerUtility
{
	...
	public LoggerUtility(CurveCraft plugin)
	{
		this.plugin = plugin;
	...
	public void log(String msg, Level TYPE)
	{
		...
		catch(Exception e)
		{
			...
			{
				plugin.getReportHandler().report(3317, "Logger doesnt work", e.getMessage(), "CurveCraft", e);
			}
		}
	}
	...
	public void log(Player p, String msg, Level TYPE)
	{
		try
		{
			...
			if(plugin.getPrivacy().getConfig().containsKey(playername))
			{
				...
				else if(plugin.getConfigHandler().getConfig().getBoolean("debug"))
				{
					System.out.println("[" + Prefix + "]" + " " + "User " + playername + " allows data");
				}
			}
			else
			{
				plugin.getLoggerUtility().log("no privacy settings for player " + playername + " found!", LoggerUtility.Level.DEBUG);
			}
			if(TYPE.equals(Level.WARNING) || TYPE.equals(Level.ERROR))
			{
				if(usePrefix)
		...
		catch(Exception e)
		{
			...
			if(plugin.getReportHandler() != null)
			{
				plugin.getReportHandler().report(3317, "PlayerLogger doesnt work", e.getMessage(), "CurveCraft", e);
			}
		}
	}
