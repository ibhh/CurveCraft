...
public class LocaleHandler extends HashMap<String, PluginLocale>
{
	...
	public LocaleHandler(CurveCraft plugin)
	{
		this.plugin = plugin;
		/**
		...
		File[] eintraege = plugin.getDataFolder().listFiles(new FileFilter()
		{
			...
			public boolean accept(File pathname)
			{
				 ...
				 */
				if(pathname.getName().startsWith("language.") && !pathname.getName().contains("language.de_DE") && !pathname.getName().contains("language.en_CA") && !pathname.getName().contains("language.en_PT"))
				{
					return true;
				}
				return false;
